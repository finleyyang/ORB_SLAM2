...
protected:
    ...
    std::vector<cv::Mat> mvX3Dc1;       //存储匹配的，当前关键帧中地图点在当前关键帧相机坐标下的坐标
    std::vector<cv::Mat> mvX3Dc2;       //存储匹配的，闭环关键帧中地图点在闭环关键帧相机坐标下的坐标
    std::vector<MapPoint*> mvpMapPoints1;   //匹配的地图点中，存储当前关键帧的地图点
    std::vector<MapPoint*> mvpMapPoints2;   //匹配的地图点中，存储闭环关键帧的地图点
    std::vector<MapPoint*> mvpMatches12;    //下标是当前关键帧中特征点的id，内容是对应匹配的，闭环关键帧中的地图点
    std::vector<size_t> mvnIndices1;    //有效的匹配关系，在vpMatched（构造函数）中的索引
    std::vector<size_t> mvSigmaSquare1;     
    std::vector<size_t> mvSigmaSquare2;
    std::vector<size_t> mvnMaxError1;   //当前关键帧中的某个特征点所允许的最大不确定性（和所在的金字塔图层有关）
    std::vector<size_t> mvnMaxError2;   //闭环关键帧中的某个特征点所允许的最大不确定性（同上）

    int N;      //下面的这个匹配关系去掉坏点和非法值之后,得到的可靠的匹配关系的点的数目
    int mN1;    //当前关键帧和闭环关键帧之间形成匹配的点的坐标

    // Current Estimation
    cv::Mat mR12i;      //存储某次RANSAC过程中得到的旋转，从闭环关键帧到当前关键帧
    cv::Mat mt12i;      //存储某次RANSAC过程中得到的平移，从闭环关键帧到当前关键帧
    float ms12i;        //存储某次RANSAC过程中得到的缩放系数，从闭环关键帧到当前关键帧
    cv::Mat mT12i;      //存储某次RANSAC过程中得到的变换矩阵，从闭环关键帧到当前关键帧
    cv::Mat mT21i;      //上面的逆
    std::vector<bool> mvbInliersi;      //内点标记，下标和N,mvpMapPoints1等一致,用于记录某次迭代过程中的内点情况
    int mnInliersi;         //内点的数量
    ...
    bool mbFixScale;           //当前传感器输入的情况下，是否需要计算尺度，单目需要，双目和RGBD不需要
    ...
    // RANSAC probability
    double mRansacProb;   // 在计算RANSAC的理论迭代次数时使用到的概率,详细解释还是看函数 SetRansacParameters() 
