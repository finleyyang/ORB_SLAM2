                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    // g2o::BlockSolver< g2o::BlockSolverTraits<6, 3> >  //每项误差项优化变量维度是6， 误差维度是3

    linearSolver = new g2o::LinearSolverEigen<g2o::BlockSolver_6_3::PoseMatrixType>(); //定义线性求解器

    g2o::BlockSolver_6_3 * solver_ptr = new g2o::BlockSolver_6_3(linearSolver);
    // 梯度下降方法，可以从GN， LM， DogLeg中选取，这里选取的是LM
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        // 前面记录maxKFid 是在这里使用的, 把地图点直接添加在位姿的后面
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        {
            // 先把优化后的位姿写入到帧的一个专门的成员变量mTcwGBA中备用
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    // 构造g2o优化器, BlockSolver_6_3表示：位姿 _PoseDim 为6维，路标点 _LandmarkDim 是3维
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            if(pFrame->mvuRight[i]<0)
            {
                ...
                Eigen::Matrix<double,2,1> obs; //观测值是两维，u，v
...
{    
    ...
    // 添加不优化的位姿顶点：固定关键帧的位姿，注意这里调用了vSE3->setFixed(true)
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    // Set Loop edges
    // 添加第1种边：闭环时因为地图点调整而出现的关键帧间的新连接关系
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        // Loop edges
        // 添加第3种边：当前帧与闭环匹配帧之间的连接关系(这里面也包括了当前遍历到的这个关键帧之前曾经存在过的回环边)
        // 获取和当前关键帧形成闭环关系的关键帧
    ...
    // 步骤5和步骤6优化得到关键帧的位姿后，地图点根据参考帧优化前后的相对关系调整自己的位置
    // 遍历所有地图点
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        int nIDr;
        // 该地图点在闭环检测在被当前KF调整过，那么使用调整它的KFid
