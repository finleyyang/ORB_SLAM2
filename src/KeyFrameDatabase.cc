...
void KeyFrameDatabase::add(KeyFrame *pKF)
{
    ...
    // vit->first指world ID,  mvInvertedFile[vit->first]包含world ID的所有关键帧
 ...
 *
 * @param[in] pKF   删除的关键帧
 * step 1: 提取要删除的关键帧的的所有的特征点的words
 * step 2: 遍历words
 * step 3: 删除每一个words对应的当前关键帧的连接
 */
...
vector<KeyFrame*> KeyFrameDatabase::DetectLoopCandidates(KeyFrame* pKF, float minScore)
{
    ...
    // 遍历闭环关键帧，挑选出共有单词数大于minCommonWords且单词匹配度大于minScore存入lScoreAndMatch
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;
